...
jest.mock('../../../src/components/Weather/WeatherContainer', () => ({
  WeatherContainer: jest.fn(({ children }) => <div data-testid="WeatherContainer">{children}</div>),
}))
...
describe('MapData', () => {
  it('MapData renders correct child components', async () => {
    ...
    const mapMarker: Marker = marker(latLng(0, 0), {})
    render(<MapData mapMarker={mapMarker} removeMarker={mockCallBack} zoomToMarker={mockCallBack} />)

    const weatherButton = screen.queryByTestId('WeatherButton')
    const weatherContainer = screen.queryByTestId('WeatherContainer')
